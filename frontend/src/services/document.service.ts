...
class DocumentService {
  ...
  async getDocumentsByGameSystem(gameSystemId: string, options: DocumentQueryOptions = {}): Promise<Document[]> {
  ...
  async uploadDocument(gameSystemId: string, data: CreateDocumentRequest): Promise<Document> {
    ...
    const response = await fetch(`${this.baseUrl}/api/game-systems/${gameSystemId}/documents`, {
      method: 'POST',
      headers: this.getAuthHeaders(),
      body: formData,
    });
  ...
  validateFile(file: File): FileValidationResult {
    ...
    if (!fileType) {
      errors.push(`Unsupported file type. Supported types: JSON, PDF, Markdown`);
    }
    ...
    if (fileType?.type === 'JSON') {
      ...
      if (file.size > 10 * 1024 * 1024) {
        warnings.push('Large JSON files may take longer to process and validate');
      }
    }
  ...
  getFileTypeFromFile(file: File): FileTypeInfo | null {
    ...
    for (const [typeName, typeInfo] of Object.entries(SUPPORTED_FILE_TYPES)) {
      if (typeInfo.extensions.includes(extension) || typeInfo.mimeTypes.includes(file.type)) {
        return {
  ...
  filterDocuments(documents: Document[], filters: {
    search?: string;
    ...
    if (filters.hasErrors !== undefined) {
      filtered = filtered.filter(doc => 
        filters.hasErrors ? doc.validationErrors.length > 0 : doc.validationErrors.length === 0
      );
    }
  ...
  sortDocuments(documents: Document[], sortBy: 'displayName' | 'createdAt' | 'updatedAt' | 'fileSize', order: 'asc' | 'desc'): Document[] {
  ...
  getTagSuggestions(documents: Document[], currentTags: string[] = []): string[] {
