...
export function EditGameSystemForm({
  ...
  onCancel,
  loading = false,
}: EditGameSystemFormProps): JSX.Element {
  const [formData, setFormData] = useState<FormData>({
    ...
    validationSchema: gameSystem.validationSchema ? JSON.stringify(gameSystem.validationSchema, null, 2) : '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    const currentSchema = gameSystem.validationSchema ? JSON.stringify(gameSystem.validationSchema, null, 2) : '';
    if (formData.validationSchema !== currentSchema) {
      submitData.validationSchema = formData.validationSchema ? JSON.parse(formData.validationSchema) : undefined;
    }
