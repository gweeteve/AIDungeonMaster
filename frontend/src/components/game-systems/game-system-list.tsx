...
export function GameSystemList({
  ...
  currentUserId,
  selectedSystemId,
}: GameSystemListProps): JSX.Element {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterType, setFilterType] = useState<'all' | 'owned' | 'derived'>('all');
  ...
  const renderEmptyState = (): JSX.Element => {
    ...
    return (
      <div className="col-span-full flex flex-col items-center justify-center py-12 text-center">
        ...
        <h3 className="text-lg font-semibold text-foreground mb-2">No game systems yet</h3>
        <p className="text-muted-foreground max-w-md mb-6">
          Get started by creating your first game system. You can build from scratch or derive from an existing system.
        </p>
        {onCreateNew && (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        {!loading && filteredSystems.map((system) => (
          <GameSystemCard
            key={system.id}
            gameSystem={system}
